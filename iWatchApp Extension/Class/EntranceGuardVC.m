...
//  iWatchApp Extension
//
//  Created by YanSY on 2018/6/26.
//  Copyright © 2018年 YanSY. All rights reserved.
//

#import "EntranceGuardVC.h"
// 导入通讯框架
#import <WatchConnectivity/WatchConnectivity.h>
#import "iWatchAPPWatchKit.h"

@interface EntranceGuardVC ()<WCSessionDelegate>
...
@implementation EntranceGuardVC

- (void)awakeWithContext:(id)context {
    [super awakeWithContext:context];
//    NSLog(@"%@__%s",[self class],__func__);
    [self setTitle:@"公司门禁"];
    [self.showMessageLabel setHidden:YES];
    //    [self pushControllerWithName:@"iWatchMainVC" context:@{@"name":@"YanSY"}];
    // Configure interface objects here.
    // 如果此菜单按钮使用自定义的图片，建议图片大小38mm手表70x70，42mm手表80x80，单位：px
    [self addMenuItemWithItemIcon:(WKMenuItemIcon)WKMenuItemIconInfo title:@"关于" action:@selector(aboutBtAction)];
}
...
- (IBAction)cancelBtAction {
    ...
    //    [self pushControllerWithName:@"iWatchMainVC" context:@{@"name":@"YanSY"}];
    //    [self presentControllerWithName:@"iWatchMainVC" context:@{@"name":@"YanSY"}];
    [WKInterfaceController reloadRootPageControllersWithNames:@[@"InterfaceController",@"iWatchMainVC"] contexts:@[@{@"name":@"YanSY"},@{@"name":@"YanSY"}] orientation:WKPageOrientationHorizontal pageIndex:1];
